...
pub async fn complete_sale(
...

    {
        ...
        let payment_amount: f64 = tx.query_row(
        ...
        let sale_items: Vec<(i32, i32)> = {
            let mut stmt = tx.prepare("SELECT product_id, quantity FROM sale_items WHERE sale_id = ?")
                .map_err(|e| format!("Sale items query error: {}", e))?;
            let rows = stmt.query_map([sale_id], |row| Ok((row.get(0)?, row.get(1)?)))
...
pub async fn create_sale_from_barcode_scan(
    ...
    let (product_id, unit_price, stock): (i32, f64, i32) = conn.query_row(
    ...
    let now = Utc::now().to_rfc3339();
    let total = unit_price * f64::from(quantity);
...
pub async fn get_daily_sales_summary(
    ...
    let (sale_count, total_sales, items_sold): (i32, f64, i32) = conn.query_row(
